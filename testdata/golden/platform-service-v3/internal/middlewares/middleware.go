...
//
// Fiber v3 note: `c` is the `fiber.Ctx` INTERFACE (v2 passed a `*fiber.Ctx`), and
// the container travels in the Fiber locals under a plain string key inside
// `kuery/ctxv3` — v3's context-key change only touched middleware-provided values,
// so it does not reach this mechanism. Nothing else about the ownership rule differs
// between the two Fiber lines.
