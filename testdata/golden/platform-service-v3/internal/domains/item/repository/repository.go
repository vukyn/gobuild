...
import (
	...
	"github.com/vukyn/testproj/internal/domains/item/entity"
	"github.com/vukyn/testproj/internal/domains/item/exceptions"
	"github.com/vukyn/testproj/internal/domains/item/models"

	// ⚠️ ctxv3, NOT kuery/ctx. This file only calls
	// pkgCtx.GetUserID(ctx context.Context), which compiles identically against
	// either package — but kuery/ctx and kuery/ctxv3 each declare their own
	// `type ContextKey string`, so a value stored under ctxv3's UserIDKey (by the
	// v3 handlers) is invisible to kuery/ctx's lookup even though both key
	// strings read "user_id". Getting this wrong builds, vets, tests and RUNS,
	// and writes an empty created_by/updated_by on every row forever, with no
	// error anywhere. The only thing that catches it is looking at the column
	// after a real write.
...
func (r *repository) List(ctx context.Context, req models.ListRequest) ([]entity.Item, int64, error) {
	...
	// Search filter. LOWER + LIKE is the portable form — it works on both SQLite
	// and Postgres; ILIKE is Postgres-only, so do not "simplify" to it.
	// (There is no upsert in this preset. If you add one, note that Postgres
	// needs the conflict target alias-qualified when the model has a bun alias.)
	if req.Search != "" {
